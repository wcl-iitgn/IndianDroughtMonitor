...
\newcommand{\idmrunhead}{%
  \begin{minipage}{\textwidth}
    {\small\bfseries\color{titleblue}{{T_doc_title}}}\hfill{\small\color{muted}{{T_init_label}}}\\[2pt]
...
PDF_STRINGS = {
    "English": {
        ...
        "footer_blurb": ("Developed by the Water and Climate Lab, IIT Gandhinagar. Forecasts are "
                         "driven by an extended-range forecast system."),
...
def _label_for(date_str):
    """'YYYY-MM-DD' -> 'Month D, YYYY' (falls back to the raw date)."""
    try:
        y, mo, d = (int(x) for x in date_str.split("-"))
        return "%s %d, %d" % (_MONTHS.get(mo, ""), d, y)
    except Exception:
        return date_str


def _latest_week(repo):
    """The current forecast initialization date = the latest week in the national
    timeseries (data/India_Drought_Area_Timeseries.txt). Returns 'YYYY-MM-DD',
...
def _rebuild_forecast_index(fdir):
    """Rebuild data/forecast_summaries/index.json from the dated forecast text
    files actually present (<param>_<YYYY-MM-DD>.txt), newest first -- the same
    way the weekly summary index is rebuilt from its summary_<date>.txt files.
    This is what lets the published forecast date advance automatically: each run
    writes the current week's text file, and the index always reflects disk."""
    dates = set()
    if fdir.is_dir():
        for p in fdir.glob("*_*.txt"):
            m = re.search(r"(\d{4}-\d{2}-\d{2})\.txt$", p.name)
            if m:
                dates.add(m.group(1))
    items = [{"date": d, "label": _label_for(d)} for d in sorted(dates, reverse=True)]
    fdir.mkdir(parents=True, exist_ok=True)
    (fdir / "index.json").write_text(json.dumps({"forecasts": items}, indent=2))
    print("  forecast index: %d date(s) on disk (newest %s)"
          % (len(items), items[0]["date"] if items else "-"))
    return items
...
def main():
    ...
    # The forecast represents the CURRENT outlook: its init date is the latest
    # week in the national timeseries, and the (date-agnostic) forecast grids
    # always hold that week's data -- so each run generates that week, which is
    # how the published date advances. --dates can override for manual
    # (re)generation of specific dates.
    if args.dates:
        gen_dates = list(args.dates)
    else:
        latest = _latest_week(repo)
        gen_dates = [latest] if latest else [dt.date.today().isoformat()]
    inits = [{"date": d, "label": _label_for(d)} for d in gen_dates if d]
    ...
    # Refresh the date index from the dated text files now on disk -- this is
    # what advances the published forecast date when a new week is generated.
    _rebuild_forecast_index(fdir)
