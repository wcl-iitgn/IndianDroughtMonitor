...
```
assets/interactive/
├── drought-map.engine.js   a factory wrapper around pranav-joshi-iitgn/IndiaDroughtMonitor/script.js:
│                           createDroughtMap(opts). Pranav's rendering/interpolation/zoom/animation
│                           logic is byte-for-byte identical; only the hardcoded canvas + file
│                           references became opts.
...
├── cdi-manifest.js         AUTO-GENERATED by build_cdi_manifest.py: window.IDM_CDI_DATES, the
│                           weekly CDI grids present in data/Drough_TS/ (drives the map date list).
